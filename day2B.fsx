...
let split (d:char) (s:string) = List.ofSeq (s.Split(d))
let splitLines = split '\n'
let splitTabs = split '\t'

let rec innerSearch (row:int list) (outer:int) (inner:int) =
    if inner = row.Length then
        0 // search failed
    else
        let pair = [
            row.[outer]
            row.[inner]]
        let max = List.max pair
        let min = List.min pair

        if max % min > 0 then
            innerSearch row outer (inner + 1)
        else
            max / min

let rec search (row:int list) (outer:int) =
    if outer = row.Length - 1 then
        0 // search failed
    else
        let quotient = innerSearch row outer (outer + 1)

        if quotient > 0 then
            quotient // search succeeded
        else
            search row (outer + 1)

let rec solve (rows:int list list) (position:int) (sum:int)  =
    if position < rows.Length then
        let row = rows.[position]
        let quotient = search row 0
        let position = position + 1
        solve rows position (sum + quotient)
    else
        sum

let input = "515	912	619	2043	96	93	2242	1385	2110	860	2255	621	1480	118	1230	99
...
[<EntryPoint>]
let main argv =
    let rows = input |> splitLines |> List.map splitTabs |> List.map (List.map int)
    printf "%A\n" (solve rows 0 0)
    0
